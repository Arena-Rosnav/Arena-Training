...
def configure_ros_logging(logging_cfg: LoggingCfg | None) -> None:
    ...
    try:
        ...
        def _severity(key: str) -> _ros_log.LoggingSeverity:
        ...
        logging.getLogger(__name__).debug("configure_ros_logging: skipped (%s)", exc)
...
def configure_trainer_logging(
    ...
    verbose: int,
    *,
    framework_logger: logging.Logger | None = None,
    trainer_logger: logging.Logger | None = None,
) -> None:
    ...
    framework_level = VERBOSE_TO_LEVEL.get(int(verbose), logging.INFO if verbose else logging.WARNING)
    if framework_logger is not None:
        framework_logger.setLevel(framework_level)
    if trainer_logger is not None:
        trainer_logger.setLevel(min(trainer_logger.level or logging.WARNING, framework_level))
